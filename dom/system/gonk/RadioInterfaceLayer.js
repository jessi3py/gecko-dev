...
RadioInterface.prototype = {
  ...
  notifyRILNetworkInterfaces: function notifyRILNetworkInterfaces(types) {
    for each (let type in types) {
      this.apnContexts.get(type).iface.notifyRILNetworkInterface();
    }
  },
  ...
  setupDataCallByType: function(apntype) {
    ...
    let dataCall = apnContext.iface.dataCall;
    if (dataCall && (dataCall.connecting ||
        dataCall.state == RIL.GECKO_NETWORK_STATE_CONNECTED)) {
      if (DEBUG) this.debug("Data call already connecting/connected.");
      return;
    }
    if (!dataCall) {
      for (let [key,value] of this.apnContexts) {
        let dc = value.iface.dataCall;
        if (dc) {
          if (dc.canHandleApn(apnContext.apnSetting)) {
            if (DEBUG) this.debug("Found compatible dc in type: " + key);
            apnContext.iface.dataCall = dc;
            break;
          }
        }
      }
      if (!apnContext.iface.dataCall) {
        if (DEBUG) this.debug("No compatible dc found, creating one.");
    ...
    if (apnContext.iface.connected) {
      ...
      apnContext.iface.notifyRILNetworkInterface();
    }
  },
  ...
  deactivateDataCallByType: function(apntype) {
    ...
    // and network manager for current data call type.
    let dataCall = apnContext.iface.dataCall;
    if (dataCall && dataCall.connected && !dataCall.inConnectedTypes(apntype)) {
      let dataInfo = this.rilContext.data;
      if (apntype == "default" && dataInfo.connected) {
        dataInfo.connected = false;
        gMessageManager.sendMobileConnectionMessage("RIL:DataInfoChanged",
...
function DataCall(radioInterface, apnSetting) {
  this.radioInterface = radioInterface;
  ...
  this.state = RIL.NETWORK_STATE_UNKNOWN;
  this.connectedTypes = [];
}
DataCall.prototype = {
  /**
  ...
  dataCallError: function(message) {
    if (message.apn != this.profile.apn) {
      return;
    }
  ...
  dataCallStateChanged: function(datacall) {
    ...
    // if APN name matched.
    if (!this.linkInfo.cid && datacall.apn != this.profile.apn) {
      return;
    }
    if (DEBUG) {
    ...
    if (this.connecting &&
      ...
      this.linkInfo.addresses = datacall.ip;
      this.linkInfo.netmask = datacall.netmask;
      this.linkInfo.broadcast = datacall.broadcast;
      this.linkInfo.gateways = datacall.gw;
      if (datacall.dns) {
        this.linkInfo.dnses = datacall.dns;
      }
    }
    ...
    if (this.state == datacall.state) {
      ...
      let changed = false;
      if (this.linkInfo.gateways != datacall.gw) {
        this.linkInfo.gateways = datacall.gw;
        changed = true;
      }
      if (datacall.dns &&
        ...
        this.linkInfo.dnses = datacall.dns[1];
        changed = true;
      }
      if (changed) {
        if (DEBUG) this.debug("Notify for data call minor changes.");
        this.radioInterface.notifyRILNetworkInterfaces(this.connectedTypes);
      }
      return;
    }
    ...
    if (this.state == RIL.GECKO_NETWORK_STATE_DISCONNECTED ||
      ...
      this.cid = null;
      removedTypes = this.connectedTypes.splice(0, this.connectedTypes.length);
      if (this.registeredAsDataCallCallback) {
        this.radioInterface.unregisterDataCallCallback(this);
        this.registeredAsDataCallCallback = false;
      }
    }

    if (removedTypes) {
      this.radioInterface.notifyRILNetworkInterfaces(removedTypes);
    } else {
      this.radioInterface.notifyRILNetworkInterfaces(this.connectedTypes);
    }
  ...
  debug: function debug(s) {
    dump("-*- DataCall[" + this.radioInterface.clientId + ":" +
         this.profile.apn + "]: " + s + "\n");
  },
  ...
  canHandleApn: function(apnSetting) {
    ...
    return (this.profile.apn == apnSetting.apn &&
            (this.profile.user || '') == (apnSetting.user || '') &&
            (this.profile.password || '') == (apnSetting.password || ''));
  },

  connect: function(apntype) {
    if (DEBUG) this.debug("connect: " + apntype);
    ...
    let authType = RIL.RIL_DATACALL_AUTH_TO_GECKO.indexOf(this.profile.authType);
    ...
    if (authType == -1) {
      if (DEBUG) {
        this.debug("Invalid authType " + this.profile.authtype);
      }
  ...
  disconnect: function(apntype) {
    if (DEBUG) this.debug("disconnect: " + apntype);
    let index = this.connectedTypes.indexOf(apntype);
    if (index == -1) {
      return
    }

    // If DataCall is still in use, just remove the apntype from connectedTypes
    // and return. Reference to the DataCall will be removed when RadioInterface
    // calls notifyRILNetworkInterface(). Otherwise, proceed with deactivation,
    // apn type will be removed from connectedTypes after data call is actually
    // disconnected.
    if (this.connectedTypes.length > 1) {
    ...
    if (DEBUG) this.debug("Going to disconnet data connection " + this.linkInfo.cid);
    this.radioInterface.deactivateDataCall(this.linkInfo.cid, reason);
  },
...
RILNetworkInterface.prototype = {
  ...
  get state() {
    if (!this.dataCall) {
      // TODO: UNKNOWN or DISCONNECTED?
      return Ci.nsINetworkInterface.NETWORK_STATE_UNKNOWN;
    }
  ...
  get ip() {
    ...
    return this.dataCall.linkInfo.addresses;
  },
  ...
  get netmask() {
    ...
    return this.dataCall.linkInfo.netmask;
  },
  ...
  get broadcast() {
    ...
    return this.dataCall.linkInfo.broadcast;
  },
  ...
  get gateway() {
    ...
    return this.dataCall.linkInfo.gateways;
  },
  ...
  get dns1() {
    ...
    return this.dataCall.linkInfo.dnses[0];
  },
  ...
  get dns2() {
    ...
    return this.dataCall.linkInfo.dnses[1];
  },
  ...
  get mmsc() {
    if (this.type != this.NETWORK_TYPE_MOBILE_MMS) {
  ...
  notifyRILNetworkInterface: function() {
    if (DEBUG) this.debug("notifyRILNetworkInterface type: " + this.type +
      ", state: " + this.state);
    ...
    if (this.connected && this.dataCall.inConnectedTypes(this.apnType) &&
        !(this.name in gNetworkManager.networkInterfaces)) {
    ...
    // This is the case where apn type is disabled but dataCall is
    // still in use, just remove the reference in this case.
    if (this.dataCall.connected && !this.dataCall.inConnectedTypes(this.apnType)) {
      if (DEBUG) this.debug("dataCall still in use, just remove reference.");
      this.dataCall = null;
      return;
    }
    // This is the case where the data call is actually disconnected,
    // so unregister network interface and remove reference.
    if (this.state == RIL.GECKO_NETWORK_STATE_DISCONNECTED &&
        this.dataCall.connectedTypes.length == 0 &&
        this.name in gNetworkManager.networkInterfaces) {
      if (DEBUG) this.debug("No one using dataCall, unregister network iface.");
  ...
  connect: function() {
    if (!this.dataCall) {
      if (DEBUG) this.debug("No dataCall on connect!");
      return;
    }
    this.dataCall.connect(this.apnType);
  },

  disconnect: function() {
    if (!this.dataCall) {
      if (DEBUG) this.debug("No dataCall on disconnect!");
      return;
    }
    this.dataCall.disconnect(this.apnType);
  },
